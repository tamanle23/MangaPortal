	...
	(function($$)
	{
		...
		$$.jsonp = function(settings)
		{
			...
			if (settings.async)
			{
				...
				script.src = settings.url.replace(RegExp("=\\?"), "="
						+ callbackName);
				$$("head").append(script);
				if (settings.timeout > 0)
				{
			...
			{
				return console
						.error("QuoJS.ajax: Unable to make jsonp synchronous call.");
			}
		};
		...
		$$.serializeParameters = function(parameters, character)
		{
			...
			for (parameter in parameters)
			{
				if (parameters.hasOwnProperty(parameter))
				{
					...
					serialize += "" + (encodeURIComponent(parameter)) + "="
...
(function()
{
	(function($$)
	{
		...
		$$.toType = function(obj)
		{
			return OBJECT_PROTOTYPE.toString.call(obj).match(/\s([a-z|A-Z]+)/)[1]
					.toLowerCase();
		};
		...
		$$.fragment = function(markup, tag)
		{
			...
			return $$.each(Array.prototype.slice.call(container.childNodes),
					function()
					{
		...
		$$.fn.filter = function(selector)
		{
			return $$([].filter.call(this, function(element)
			{
				return element.parentNode
						&& $$.query(element.parentNode, selector).indexOf(
								element) >= 0;
			}));
		};
...
(function()
{
	(function($$)
	{
		...
		$$.Event = function(type, touch)
		{
			...
			event.initEvent(type, true, true, null, null, null, null, null,
				...
				_subscribe(element, event, callback, selector, function(fn)
				{
					return function(e)
					{
						...
						if (match)
						{
							...
							return fn.apply(match, [ evt ].concat([].slice
									.call(arguments, 1)));
						}
					};
				});
		...
		_subscribe = function(element, event, callback, selector,
				delegate_callback)
		{
			...
			element_handlers = HANDLERS[element_id]
					|| (HANDLERS[element_id] = []);
		...
		_unsubscribe = function(element, event, callback, selector)
		{
			...
			event = _environmentEvent(event);
			element_id = _getElementId(element);
			return _findHandlers(element_id, event, callback, selector)
					.forEach(
							function(handler)
							{
								delete HANDLERS[element_id][handler.index];
								return $$.fn.removeEvent(element,
										handler.event, handler.proxy);
							});
		};
...
(function()
{
	(function($$)
	{
		...
		HOLD_DELAY = 650;
		GESTURES = [ "touch", "tap", "singleTap", "doubleTap", "hold", "swipe",
				"swiping", "swipeLeft", "swipeRight", "swipeUp", "swipeDown",
				"rotate", "rotating", "rotateLeft", "rotateRight", "pinch",
				"pinching", "pinchIn", "pinchOut", "drag", "dragLeft",
		...
		GESTURES.forEach(function(event)
		{
			$$.fn[event] = function(callback)
			{
				...
				return $$(document.body).delegate(this.selector, event_name,
						callback);
			};
			return this;
		});
		...
		_listenTouches = function()
		{
			...
			{
			    environment.bind("MSPointerDown", _onTouchStart);
			    environment.bind("MSPointerMove", _onTouchMove);
			    environment.bind("MSPointerUp", _onTouchEnd);
			}
			else
			{
		...
		_isSwipe = function(event)
		{
			...
			if (CURRENT_TOUCH[0])
			{
				move_horizontal = Math.abs(FIRST_TOUCH[0].x
						- CURRENT_TOUCH[0].x) > 30;
		...
		_onTouchEnd = function(event)
		{
			...
			if (GESTURE.fingers === 1)
			{
				...
				{
					_trigger("swipe");
					swipe_direction = _swipeDirection(FIRST_TOUCH[0].x,
							CURRENT_TOUCH[0].x, FIRST_TOUCH[0].y,
							CURRENT_TOUCH[0].y);
			...
			} else
			{
				...
				if (GESTURE.angle_difference !== 0)
				{
					...
					rotation_direction = GESTURE.angle_difference > 0 ? "rotateRight"
							: "rotateLeft";
				...
				if (GESTURE.distance_difference !== 0)
				{
					...
					pinch_direction = GESTURE.distance_difference > 0 ? "pinchOut"
							: "pinchIn";
				...
				{
					if (Math.abs(FIRST_TOUCH[0].x - CURRENT_TOUCH[0].x) > 10
							|| Math.abs(FIRST_TOUCH[0].y - CURRENT_TOUCH[0].y) > 10)
					{
						_trigger("drag");
						drag_direction = _swipeDirection(FIRST_TOUCH[0].x,
								CURRENT_TOUCH[0].x, FIRST_TOUCH[0].y,
								CURRENT_TOUCH[0].y);
		...
		_fingersPosition = function(touches, fingers)
		{
			...
			result = [];
			i = 0;
			touches = touches[0].targetTouches ? touches[0].targetTouches
					: touches;
			while (i < fingers)
			{
				result.push({
		...
		_captureRotation = function()
		{
			...
			if (Math.abs(diff) > 20 || GESTURE.angle_difference !== 0)
			{
				...
				while (Math.abs(diff - GESTURE.angle_difference) > 90
						&& i++ < 10)
				{
		...
		_trigger = function(type, params)
		{
			if (GESTURE.el)
			{
				...
				if (CURRENT_TOUCH[0])
				{
					params.iniTouch = (GESTURE.fingers > 1 ? FIRST_TOUCH
							: FIRST_TOUCH[0]);
					params.currentTouch = (GESTURE.fingers > 1 ? CURRENT_TOUCH
							: CURRENT_TOUCH[0]);
				}
...
(function()
{
	(function($$)
	{
		...
		$$.fn.parent = function(selector)
		{
			...
			ancestors = (selector ? _findAncestors(this) : this
		...
		$$.fn.siblings = function(selector)
		{
			...
			{
				return Array.prototype.slice.call(element.parentNode.children)
		...
		_findAncestors = function(nodes)
		{
			...
			while (nodes.length > 0)
			{
				...
				{
					if ((node = node.parentNode) && node !== document
		...
		$$.fn.style = function(property, value)
		{
			...
			} else
			{
				return this[0].style[property]
						|| _computedStyle(this[0], property);
			}
		};
